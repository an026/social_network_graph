...
class TestMongoBridge(TestCase):
    ...
    def setUp(self) -> None:
        self.mongo_bridge = MongoBridge("mongodb://localhost:27017/", "Data", "Artists")
        # setUp
        collaborators = [{"collaboratorName": "Barbara",
                          "collaboratorID": "T?",
                          "releaseID": 1998,
                          "roles": ["Spouse", "Colleague"]}]
        self.test_artist_dictionary = {"artistName": "Jerry",
                                  "artistID": -237417,
                                  "realname": "Gerald",
                                  "profile": "",
    ...
    def test_get_no_artists(self):
        ...
        with self.assertRaises(ArtistNotFound):
            self.mongo_bridge = MongoBridge("mongodb://localhost:27017/", "Data", "NoArtists")
            artists = self.mongo_bridge.get_all_artists()
    ...
    def test_add_artist_by_dict(self):
        ...
        self.assertEqual("Jerry", local_artist["artistName"])
        self.assertEqual("Gerald", local_artist["realname"])
        self.assertEqual(42, local_artist["level"])
        self.assertEqual("", local_artist["profile"])
        # Tear this down immediately
        mongoclient = pymongo.MongoClient("mongodb://localhost:27017/")
        mycollection = mongoclient["Data"]["Artists"]
